...
def train(model, optimizer, use_cuda, train_dataloader, epoch, log_interval=100, grad_mask=None, multi_gpu=False):
    if multi_gpu:
        model = nn.DataParallel(model.cuda())
    model.train()
    iteration = 0
    ...
    for batch_idx, (data, target) in enumerate(train_dataloader):
        ...
        if grad_mask is not None:
            for module in grad_mask.keys():
                module.weight.grad *= grad_mask[module]
        optimizer.step()
        pred = output.data.max(1, keepdim=True)[1]
        correct_batch = pred.eq(target.data.view_as(pred)).sum().item()
        correct += correct_batch
