...
class PruneResults:
    ...
    def _compute_basic_stats(self):
        ...
        if self.pruner.skip:
            ...
            train_acc, train_loss = validation(model=self.pruner.model, use_cuda=torch.cuda.is_available(),
                                               val_dataloader=self.train_dataloader)
            val_acc, val_loss = validation(model=self.pruner.model, use_cuda=torch.cuda.is_available(),
                                           val_dataloader=self.val_dataloader)
