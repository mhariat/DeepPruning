...
from prune.channels_pruning.methods.taylor import *
from prune.channels_pruning.methods.obd import *
from prune.channels_pruning.methods.obs import *
from prune.channels_pruning.methods.norm import *
from prune.weights_pruning.methods.magnitude import *
from prune.low_rank_pruning.methods.eigen_damage import *
...
pruner_id = {
    ...
    # Low Rank pruning
    5: 'Eigen-Damage',
    6: 'Improved-Eigen-Damage',
    7: 'Improved-Eigen-Damage_multi',
    # Weight pruning
    8: 'Magnitude',
}
...
def init_pruner(config, network):
    ...
    if config.pruner_id in [0, 1, 2, 3, 4]:
        if 'use_out_dependencies' in config:
            kwargs.update({'use_out_dependencies': config.use_out_dependencies})
    if config.pruner_id == 0:
        pruner = TaylorPruner(**kwargs)
    elif config.pruner_id in [1, 2]:
        if 'norm' in config:
            kwargs.update({'norm': config.norm})
        pruner = NormPruner(**kwargs)
    elif config.pruner_id == 3:
        pruner = OBSPruner(**kwargs)
    elif config.pruner_id == 4:
        pruner = OBDPruner(**kwargs)
    elif config.pruner_id in [5, 6, 7]:
        ...
        if config.pruner_id == 5:
            pruner = EigenPruner(**kwargs)
        else:
...
def main(config):
    ...
    net, exp_name, original_epochs = init_network(config, num_classes)
    ...
    momentum, weight_decay = config.momentum, config.weight_decay
    ...
    while not stop_pruning:
        ...
        pruner.fine_tune(epochs=epochs, optimizer=optimizer, scheduler=scheduler, train_dataloader=train_dataloader,
                         val_dataloader=val_dataloader, distillation=config.distillation, alpha=config.alpha,
