...
class Pruner(metaclass=ABCMeta):
    def __init__(self, model, prune_ratio_limit, normalize, log_interval, use_hook):
        ...
        self.saved_model = []
        self.it = 0
        self.skip = ('skip' in self.model.name.lower())
    ...
    def fine_tune(self, epochs, optimizer, scheduler, train_dataloader, val_dataloader, logger=None,
                  distillation=False, alpha=None, temperature=None):
        ...
        for epoch in range(epochs):
            if self.skip:
                ...
                if distillation:
                    teacher = self.saved_model[-1][1]
                    train_accuracy, train_loss = train_kd(student=self.model, teacher=teacher,
                                                          optimizer=optimizer, use_cuda=self.use_cuda,
                                                          train_dataloader=train_dataloader, epoch=epoch, alpha=alpha,
                                                          temperature=temperature, log_interval=100)
                else:
                    train_accuracy, train_loss = train(model=self.model, optimizer=optimizer, use_cuda=self.use_cuda,
                                                       train_dataloader=train_dataloader, epoch=epoch,
                                                       log_interval=self.log_interval, grad_mask=self.grad_mask,
                                                       multi_gpu=multi_gpu)
                val_accuracy, val_loss = validation(model=self.model, use_cuda=self.use_cuda,
                                                    val_dataloader=val_dataloader)
                message = 'Fine-tuning. Epoch: [{}/{}]. Train Loss: {:.6f}. Train Accuracy: {:.2f}%.' \
                          ' Validation loss: {:.6f}. Validation Accuracy: {}/{} ({:.2f}%).' \
                          ' Current learning rate: {:.4f}. (started at: {:.4f})'.\
                    format(epoch+1, epochs, train_loss, 100 * train_accuracy, val_loss,
                           int(val_accuracy*len(val_dataloader.dataset)), len(val_dataloader.dataset),
                           100*val_accuracy, lr, starting_lr)
        ...
        if self.grad_mask is not None:
            self.grad_mask = OrderedDict()
        self.saved_model.append((val_accuracy, copy.deepcopy(self.model).cpu()))
