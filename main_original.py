...
def init_network(args, num_classes):
    ...
    if args.network == 'vgg':
        ...
        if 'cifar' in dataset.lower():
            kwargs.update({'dataset': 'cifar'})
        kwargs.update({'cardinality': args.cardinality, 'base_width': args.base_width})
        net = resnext(**kwargs)
...
def main():
    ...
    parser.add_argument('--avg_pool2d', type=int, default=7, help='Average pool kernel size for vgg')
    parser.add_argument('--depth', type=int, help='Depth of the netowrk')
    parser.add_argument('--cardinality', type=int, default=8, help='cardinality of the netowrk')
    parser.add_argument('--base_width', type=int, help='Base width of the netowrk')
    ...
    parser.add_argument('--alpha', type=float, default=0, help='Log interval')
    ...
    skip = ('skip' in net.name.lower())
    create_dir(args.result_dir)
    dataset = args.data_dir.split('/')[-1]
    path_to_add = '{}/{}/{}'.format(dataset, args.network, depth)
    writer_dir = '{}/writer/original/{}'.format(args.result_dir, path_to_add)
    ...
    checkpoint_dir = '{}/checkpoint/original/{}'.format(args.result_dir, path_to_add)
    log_dir = '{}/logs/original/{}'.format(args.result_dir, path_to_add)
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        print(message)
        if skip:
            train_acc, train_loss = train_sp(net, optimizer, use_cuda, train_dataloader, epoch, alpha=args.alpha,
            ...
            message = 'Training. Epoch: [{}/{}]. Train Loss: {:.6f}. Train Accuracy: {:.2f}%. ' \
                      'Validation loss: {:.6f}. Validation Accuracy: {}/{} ({:.2f}%). ' \
                      'Current Learning rate: {:.5f}. (started at: {:.4f}). Computation Percentage: {:.3f} %'. \
                format(epoch + 1, start_epoch + epochs, train_loss, 100 * train_acc, val_loss, int(100 * val_acc),
                       len(val_dataloader.dataset), 100. * val_acc, lr, starting_lr, cp)
        else:
            train_acc, train_loss = train(net, optimizer, use_cuda, train_dataloader, epoch, args.log_interval)
            val_acc, val_loss = validation(model=net, use_cuda=use_cuda, val_dataloader=val_dataloader)
            ...
            message = 'Training. Epoch: [{}/{}]. Train Loss: {:.6f}. Train Accuracy: {:.2f}%. Learning rate {:.5f}.' \
                ...
                format(epoch + 1, start_epoch + epochs, train_loss, 100 * train_acc, lr, val_loss, int(100*val_acc),
                       len(val_dataloader.dataset), 100. * val_acc)
